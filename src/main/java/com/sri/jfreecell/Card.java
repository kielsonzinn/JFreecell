 ...
 * Issues:<br>
 * Fragile: This loads each card image from a file, and has the file-naming
 * conventions for the card built into it <br>
...
public class Card extends CardTimeline implements Serializable {
   ...
   public boolean isVisibleInside(int x, int y, boolean isTopCard) {
      return (x >= this.x && x < this.x + CARD_WIDTH)
            && (y >= this.y && (isTopCard ? y < this.y + CARD_HEIGHT : y < this.y + 15));
   }
